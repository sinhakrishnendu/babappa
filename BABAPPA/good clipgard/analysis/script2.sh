...
process_species() {
    ...
    # Find the species-specific MSA file. The file must have the pattern "${species}.fas".
    local msa_file
    msa_file=$(find "$msa_dir" -name "${species}.fas" -print -quit)
...
# It lists all files matching *.fas and strips the directory and suffix to leave the species name.
species_list=$(ls "$msa_dir"/*.fas | sed 's|.*/||' | sed 's|\.fas$||')
